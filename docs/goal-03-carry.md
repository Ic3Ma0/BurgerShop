...
Goal 03 交付时尚无顾客；Goal 04 增加顾客和点单台，Goal 05 已接入金色区域自动送餐。交付使用 `TryTakeBurger(out Transform)` 将原汉堡模型和材质交给顾客；无参数版本仍用于直接消耗。
...
PR #8 已合并 main。用户于 2026-09-11 试玩后确认“效果很不错”。Goal 04 开发期间再次回归本阶段测试，全部通过；并修复连续场景测试退出 Play Mode 时，临时输入设置对象被销毁导致的编辑器断言。
