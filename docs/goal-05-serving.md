...
金币暂为本次运行收入，重开 Play 会归零。本分支的 Goal 06 已支持紫色区域购买烤台升级，存档属于 Goal 08。
...
- `RestaurantWallet` 保存收入和成交笔数，拒绝非正金额，并通过 `SaleRecorded` 通知音效。后续 Goal 06 已增加 `TrySpend` 和 `CoinsSpent`，购买升级会扣除余额，但不减少成交笔数；尚无持久化。
...
上述为 Goal 05 的 Editor 验证；Goal 06 已进一步验证赚取金币后购买升级。存档、Android 构建、真机触控及长时间性能仍未验收。

## 集成状态

Goal 04 / 05（PR #9 / #10）均已集成至 main。Goal 06 开发前已同步并核对主线 `f40b77a`，内容与原 Goal 05 验收版本一致。
