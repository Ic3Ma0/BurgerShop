...
namespace BurgerShop.Editor
{
    ...
    public static class FacilityCatalogBaker
    {
        [MenuItem("BurgerShop/Rebuild supermarket photos")]
        public static void Bake()
        {
            if(EditorApplication.isPlaying)throw new System.InvalidOperationException("Stop Play before rebuilding catalog photos.");
            string directory=Path.Combine(Application.dataPath,"_Project/Resources/FacilityCatalog");
            ...
            var actor=new GameObject("PhotoDependencies");actor.transform.SetParent(inactive.transform,false);
            var player=actor.AddComponent<BurgerInventory>();player.Configure();
            var wallet=actor.AddComponent<RestaurantWallet>();var parts=actor.AddComponent<PartsWallet>();
            var cash=actor.AddComponent<CashFloor>();var dining=actor.AddComponent<DiningArea>();
            ...
            try
            {
                for(int index=0;index<=FacilityCatalog.Offers.Count;index++)
                {
                    bool roomPhoto=index==FacilityCatalog.Offers.Count;
                    string key=roomPhoto?"Restroom":FacilityCatalog.Offers[index].Kind.ToString();
                    Transform source;
                    if(roomPhoto)
                    {
                        source=new GameObject("RestroomPhotoModel").transform;source.SetParent(inactive.transform,false);
                        var room=source.gameObject.AddComponent<RestroomExpansion>();room.Configure(wallet,player,null);room.Restore(true,0);
                        foreach(var hinge in source.GetComponentsInChildren<Transform>(true))if(hinge.name=="CubicleDoorHinge")hinge.localRotation=Quaternion.Euler(0,80,0);
                    }
                    else source=FacilityFactory.Create(inactive.transform,FacilityCatalog.Offers[index].Kind,"photo:"+key,player,wallet,parts,cash,dining).transform;
                    camera.transform.rotation=roomPhoto?Quaternion.Euler(35,145,0):Quaternion.Euler(27,-35,0);
                    ...
                    foreach(var renderer in source.GetComponentsInChildren<MeshRenderer>(true))
                    {
                        ...
                        if(!VisibleModel(renderer,source.transform,roomPhoto))continue;
            ...
            {
                string path="Assets/_Project/Resources/FacilityCatalog/"+Path.GetFileName(file);
            ...
            Debug.Log("Supermarket: exported all "+FacilityCatalog.Offers.Count+" facility photos and the restroom expansion.");
        }
