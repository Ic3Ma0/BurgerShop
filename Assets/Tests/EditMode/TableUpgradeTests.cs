...
namespace BurgerShop.Tests.EditMode
{
    public sealed class TableUpgradeTests
    {
        ...
        public void InvestingEightyOpensTableChangePopupWithoutRaisingPayYet()
        {
            ...
            Assert.That(hud.Popup.TitleLabel.text, Is.EqualTo("更换餐桌"));
            Assert.That(hud.Popup.LevelLabel.text, Is.EqualTo("等级 2 · +2 ★"));
            Assert.That(copy, Does.Contain("小馆款"));
            Assert.That(copy, Does.Contain("快餐款"));
            Assert.That(copy, Does.Contain("庭院款"));
            Assert.That(copy, Does.Contain("实惠款"));
            Assert.That(copy, Does.Contain("高效款"));
            Assert.That(copy, Does.Contain("高收益款"));
            Assert.That(copy, Does.Contain("12 / 每餐"));
            Assert.That(copy, Does.Contain("16 / 每餐"));
            Assert.That(copy, Does.Contain("2.4秒"));
            Assert.That(copy, Does.Contain("3.6秒"));
            ...
            Assert.That(hud.Popup.SelectLabels[2].text.ToLowerInvariant(), Does.Contain("还差"));
            Assert.That(copy, Does.Contain("选择"));
