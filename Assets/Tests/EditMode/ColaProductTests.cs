...
namespace BurgerShop.Tests.EditMode
{
    public sealed class ColaProductTests
    {
        ...
        [Test]
        public void ColaMachineAndCounterSitInTheBackWingWithStarterColors()
        {
            ...
            Assert.That(ShopLayout.Horizontal(ShopLayout.Cola, new Vector3(9f, 0f, 3f)), Is.GreaterThan(10f));
            Assert.That(ShopLayout.Horizontal(ShopLayout.ColaCounter, new Vector3(3f, 0f, 4f)), Is.GreaterThan(10f));
            ...
            Assert.That(pickup.g, Is.GreaterThan(pickup.r), "green pickup");
            Assert.That(pickup.g, Is.GreaterThan(pickup.b), "green pickup");
            Assert.That(upgrade.b, Is.GreaterThan(upgrade.g), "purple upgrade");
            Assert.That(upgrade.r, Is.GreaterThan(upgrade.g), "purple upgrade");
            ...
            Assert.That(white.r, Is.GreaterThan(0.9f), "white cashier");
            Assert.That(white.g, Is.GreaterThan(0.9f), "white cashier");
            Assert.That(white.b, Is.GreaterThan(0.9f), "white cashier");
            Color hireHud = new Color(0.23f, 0.83f, 0.94f);
            Assert.That(hireHud.b, Is.GreaterThan(hireHud.r), "cyan hire HUD must stay distinct");
            Assert.That(Mathf.Abs(pickup.b - hireHud.b), Is.GreaterThan(0.2f));
        }
        ...
        [Test]
        public void CarryHudAddsColaLineAndTaskCapsuleAsksForTheColaCounter()
        {
            colaMachine.Station.Advance(12f);
            inventory.TryCollectFrom(colaMachine.Station);
            var canvas = new GameObject("Carry", typeof(RectTransform));
            canvas.transform.SetParent(root.transform, false);
            Text label = canvas.AddComponent<Text>();
            canvas.AddComponent<CarryHud>().Configure(inventory, colaMachine.Pickup, label);
            Assert.That(label.text, Does.Contain("COLA 1"));
            Assert.That(label.text, Does.Not.Contain("GRILL"));
            ...
            goals.Configure(inventory, burgerGrill.Station, burgerStock, root.GetComponent<CustomerQueue>(), wallet,
                burgerServing, DiningArea.Wrap(table), null, hiring, null, null, colaServing, colaMachine.Station);
            goals.Advance(2f);
            Assert.That(goals.Title, Is.EqualTo("Move to cola counter"));
        }
        ...
        public void EmptyHandsStillAskToPickUpABurgerWhenBothMachinesHaveStock()
        {
            ...
            goals.Advance(2f);
            Assert.That(goals.Title, Is.EqualTo("Pick up a burger"));
        }
        ...
        [Test]
        public void PersistenceWritesVersion10ColaThroughTheWingExpansion()
        {
            string directory = Path.Combine(Path.GetTempPath(), "BurgerShopCola-" + System.Guid.NewGuid().ToString("N"));
            try
            {
                DiningArea dining = DiningArea.Wrap(table);
                ShopExpansion expansion = ShopExpansion.Create(root.transform, dining, burgerServing, hiring,
                    inventory, wallet);
                expansion.Restore(false, false, false, 0, false, false, false, false, true, true, true, 2);
                Assert.That(expansion.HasWing, Is.True);
                Assert.That(expansion.HasColaMachine, Is.True);
                Assert.That(expansion.ColaLevel, Is.EqualTo(2));
                ...
                persistence.Configure(wallet, burgerGrill.Upgrade, hiring, null, expansion, directory);
                ...
                Assert.That(data.boughtSideWing, Is.True);
                Assert.That(data.boughtColaMachine, Is.True);
                Assert.That(data.boughtColaCounter, Is.True);
                ...
                {
                    version = 9, coins = 600, completedSales = 18, grillLevel = 3, workerHired = false,
                ...
                expansion = ShopExpansion.Create(root.transform, dining, burgerServing, hiring, inventory, wallet);
