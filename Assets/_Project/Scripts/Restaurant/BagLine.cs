...
namespace BurgerShop.Restaurant
{
    public sealed class BagLine : MonoBehaviour
    {
        ...
        {
            if(Expanded||goals==null||goals.Rank<6)return false;
        ...
        void BuildMachine()
        {
            ...
            ShopFixtures.CreateActionCircle(area,"BagCollect",MachinePoint,HudChrome.Green);
        ...
        void BuildTable()
        {
            ...
            ShopFixtures.CreateActionCircle(area,"BagWork",WorkPoint,HudChrome.Green);
        ...
        void BuildCounter()
        {
            ...
            ShopFixtures.CreateActionCircle(area,"PickupServe",CounterPoint,Color.white);
            ...
            Queue=new GameObject("PickupQueue").AddComponent<CustomerQueue>();Queue.transform.SetParent(area,false);
        ...
        public void Advance(float seconds)
        {
            ...
            while(seconds>.00001f)
            {
                ...
                {
                    produced+=dt;if(produced>=ProductionSeconds){produced-=ProductionSeconds;var item=BagVisualFactory.Create(machineModel,false);empty.Add(item);Restack(empty,Machine+new Vector3(.8f,.2f,0),.1f);}
                }
                TickActor(player,dt);
                ...
                if(TableBuilt&&staffed)
                {
                    if(processing)
                    {
                        ...
                        if(processAge>=ProcessingSeconds){output.Add(inProcess);inProcess=null;processing=false;Restack(output,Workbench+new Vector3(.7f,1,0),.22f);}
                    }
                    if(!processing&&raw.Count>0&&bags.Count>0&&output.Count<8)
                    {
                        DestroyLast(raw);DestroyLast(bags);processing=true;processAge=0;
                        inProcess=BagVisualFactory.Create(tableModel,true);inProcess.position=Workbench+Vector3.up*1.1f;
                    }
                }
        ...
        bool AtWork(BurgerInventory actor)=>actor!=null&&actor.isActiveAndEnabled&&ShopLayout.Horizontal(actor.transform.position,WorkPoint)<=.9f;
        void TickActor(BurgerInventory actor,float dt)
        {
            ...
            bool moved=false;
            if(MachineBuilt&&ShopLayout.Horizontal(actor.transform.position,MachinePoint)<=.9f&&empty.Count>0&&!actor.IsFull)
            ...
            else if(TableBuilt&&AtWork(actor))
            {
                ...
                {raw.Add(burger);burger.SetParent(tableModel,true);Restack(raw,Workbench+new Vector3(-.5f,1,0),.12f);actor.GetComponent<RestaurantWorker>()?.RawDeposited(1);moved=true;}
                else if(actor.EmptyBagCount>0&&bags.Count<8&&actor.TryTake(CarriedItemKind.EmptyBag,out var bag))
                {bags.Add(bag);bag.SetParent(tableModel,true);Restack(bags,Workbench+new Vector3(0,1,0),.12f);moved=true;}
            ...
            else if(CounterBuilt&&ShopLayout.Horizontal(actor.transform.position,CounterPoint)<=.9f)
            {
                if(stock.Count<8&&actor.TryTake(CarriedItemKind.Bagged,out var item))
                {stock.Add(item);item.SetParent(counterModel,true);Restack(stock,Counter+Vector3.up,.18f);moved=true;}
                TryServe(actor);
            }
        ...
        {
            if(!ReadyToSell||actor==null||ShopLayout.Horizontal(actor.transform.position,CounterPoint)>.9f||!wallet.CanCompleteSale())return false;
            var c=Queue.ReadyCustomer;if(!c.Order.TryReserve())return false;
            flightCustomer=c;flightItem=stock[stock.Count-1];stock.RemoveAt(stock.Count-1);flightOrigin=flightItem.position;flightItem.SetParent(c.transform,true);flightAge=0;flightWorker=actor.GetComponent<RestaurantWorker>();return true;
        }
        void AdvanceSale(float dt)
        {
            if(flightCustomer==null)return;flightAge+=dt;float t=Mathf.Clamp01(flightAge/.45f);
            flightItem.position=Vector3.Lerp(flightOrigin,flightCustomer.transform.position+Vector3.up,t)+Vector3.up*Mathf.Sin(t*Mathf.PI)*.5f;
            if(t<1)return;
            ...
            if(c.Order.TrySettle())
            {
                ...
                c.BeginDeparture(flightItem,new[]{new Vector3(-26,0,-8),new Vector3(-18,0,-8)},money,null,true);
                wallet.RecordCompletedSale();cash?.DropAt(Counter+new Vector3(1,0,-1),money);CompletedOrders++;
        ...
        void Update()
        {
            if(!Expanded&&goals!=null&&goals.Rank>=6&&expandLabel==null)
