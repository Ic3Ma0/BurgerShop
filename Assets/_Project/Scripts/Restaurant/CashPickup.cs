...
namespace BurgerShop.Restaurant
{
    public sealed class CashPickup : MonoBehaviour
    {
        ...
        public const float FlyDuration = 0.35f;
        public const float BillWidth = 0.42f;
        public const float BillThickness = 0.03f;
        public const float BillDepth = 0.26f;
        public const float FaceTilt = 18f;
        public const float BobAmplitude = 0.055f;
        public const float BobRadians = 2.6f;
        public const float SpinDegreesPerSecond = 32f;
        public static readonly Color BillColor = new Color(0.42f, 0.98f, 0.52f);
        public static readonly Color StripeColor = new Color(0.22f, 0.92f, 0.40f);
        ...
        public static CashPickup Create(Transform parent, Vector3 position, int amount, int stackIndex)
        {
            ...
            root.rotation = Quaternion.Euler(0f, stackIndex * 12f, 0f);
        ...
        public void LaunchTo(Transform collector, System.Action arrived)
        {
            ...
            Motion.Launch(collector, new Vector3(0f, 1.45f, 0.1f), Vector3.up * 0.4f, Vector3.one * 0.35f, arrived);
        }
        ...
        static Transform BuildBills(Transform parent, int index)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                int row = i / 2;
                Part(visual, "Bill_" + i,
                    new Vector3((col - 0.5f) * 0.46f, 0.016f + i * 0.005f, (row - 1f) * 0.32f),
                    new Vector3(BillWidth, BillThickness, BillDepth),
                    i % 3 == 0 ? stripe : bill);
            }
